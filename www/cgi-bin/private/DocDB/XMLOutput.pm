...
sub RevisionXMLOut {
  ...
  if ($XMLDisplay{All} || $XMLDisplay{Title}) {
    XML::Twig::Elt -> new("title" => {'#CDATA' => 1}, $DocRevisions{$DocRevID}{Title}) -> paste(first_child => $RevisionXML);
  }

  if ($XMLDisplay{All} || $XMLDisplay{DCCNumber}) {
    XML::Twig::Elt -> new("dccnumber", $Documents{$DocumentID}{Alias}) -> paste(first_child => $RevisionXML);
  }
  ...
  if ($XMLDisplay{All} || $XMLDisplay{AuthorGroups}) {
    require "AuthorSQL.pm";
    require "AuthorUtilities.pm";
    ...
    foreach my $AuthorGroupID (@AuthorGroupIDs) {
      my $AuthorGroupXML = AuthorGroupXMLOut( {-authorgroupid => $AuthorGroupID} );
      if ($AuthorGroupXML) {
        $AuthorGroupXML -> paste(last_child => $RevisionXML);
      }
    }
  }
  ...
  if ($XMLDisplay{All} || $XMLDisplay{Abstract}) {
    my $AbstractXML = XML::Twig::Elt -> new("abstract" => { '#CDATA' => 1}, $DocRevisions{$DocRevID}{Abstract});
  ...
  if ($XMLDisplay{All} || $XMLDisplay{Note}) {
    my $NoteXML = XML::Twig::Elt -> new("note" => { '#CDATA' => 1}, $DocRevisions{$DocRevID}{Note});
  ...
  if ($XMLDisplay{All} || $XMLDisplay{PubInfo}) {
    my $PubInfoXML = XML::Twig::Elt -> new("publicationinfo" => { '#CDATA' => 1}, $DocRevisions{$DocRevID}{PUBINFO});
...
sub DocTypeXMLOut {
  ...
  my $Short      = XML::Twig::Elt -> new("name"  => {'#CDATA' => 1},       Printable($DocumentTypes{$DocTypeID}{SHORT}));
  my $Long       = XML::Twig::Elt -> new("description"  => {'#CDATA' => 1},Printable($DocumentTypes{$DocTypeID}{LONG}));
...
sub AuthorXMLOut {
  ...
  my $EmployeeNumber = FetchEmployeeNumberFromAuthorID($AuthorID);
  ...
  if ($EmployeeNumber) {
      my $Employee   = XML::Twig::Elt -> new("employeenumber", $EmployeeNumber); 
      $Employee -> paste(last_child => $AuthorXML);
  }
...
sub AuthorGroupXMLOut {
  my ($ArgRef) = @_;
  my $AuthorGroupID    = exists $ArgRef->{-authorgroupid}    ? $ArgRef->{-authorgroupid}    : 0;
  require "AuthorSQL.pm";

  my $ElementName = "authorgroup";

  unless ($AuthorGroupID && FetchAuthorGroup($AuthorGroupID)) {
    return undef;
  }

  my %Attributes = ();
  $Attributes{id} = $AuthorGroupID;

  my $AuthorGroupXML = XML::Twig::Elt -> new($ElementName => \%Attributes );
  my $Name     = XML::Twig::Elt -> new("name",Printable($AuthorGroups{$AuthorGroupID}{AuthorGroupName}));

  $Name -> paste(last_child => $AuthorGroupXML);

  return $AuthorGroupXML;
}
...
sub TopicXMLOut {
  ...
  my $Short    = XML::Twig::Elt -> new("name"  => {'#CDATA' => 1},       Printable($Topics{$TopicID}{Short}));
  my $Long     = XML::Twig::Elt -> new("description"  => {'#CDATA' => 1},Printable($Topics{$TopicID}{Long}));
...
sub EventXMLOut {
  ...
  my $Name        = XML::Twig::Elt -> new("name"  => {'#CDATA' => 1},        Printable($Conferences{$EventID}{Title}));
  my $Location    = XML::Twig::Elt -> new("location"  => {'#CDATA' => 1},    Printable($Conferences{$EventID}{Location}));
  my $Description = XML::Twig::Elt -> new("description"  => {'#CDATA' => 1}, Printable($Conferences{$EventID}{LongDescription}));
...
sub XRefToXMLOut {
  ...
  foreach my $DocXRefID (@DocXRefIDs) {
    ...
    my $Alias = FetchDocumentAlias($DocXRefs{$DocXRefID}{DocumentID});
    my %Attributes = ();
...
sub XRefByXMLOut {
  ...
  foreach my $DocXRefID (@DocXRefIDs) {
    ...
    my $DocumentID = $DocRevisions{$DocRevID}{DOCID};
    my $Alias = FetchDocumentAlias($DocumentID);
    # my $Alias = FetchDocumentAlias($DocXRefs{$DocXRefID}{DocumentID});
...
sub FileXMLOut {
  ...
  foreach my $DocFileID (@DocFileIDs) {
    ...
    my $Name        = XML::Twig::Elt -> new("name"  => {'#CDATA' => 1}, Printable($DocFiles{$DocFileID}{Name}));
    my $Description = XML::Twig::Elt -> new("description" => { '#CDATA' => 1}, $DocFiles{$DocFileID}{DESCRIPTION});
...
sub SecurityXMLOut {
  ...
  foreach my $ID (@IDs) {
    ...
    my $Name        = XML::Twig::Elt -> new("name"  => {'#CDATA' => 1},        Printable($SecurityGroups{$ID}{NAME}));
    my $Description = XML::Twig::Elt -> new("description"  => {'#CDATA' => 1}, Printable($SecurityGroups{$ID}{Description}));
