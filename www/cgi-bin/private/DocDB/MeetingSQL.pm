...
sub GetRevisionEvents ($%) { # Get the events associated with a revision
  ...
  my $DocRevID = shift;
  my %Params= @_;
  # FIXME: variable name is misnomer: this flags triggers filtering list
  # of ConferenceID based on user's privileges and events ACL 
  my $AccessibleEventsOnly =  $Params{-accessible_only} //  0;
  require "Utilities.pm";
  ...
  if ($AccessibleEventsOnly )  {
     use MeetingSecurityUtilities;
     @ConferenceIDs = grep { &CanAccessMeeting($_) } @ConferenceIDs ;     
  }
  return @ConferenceIDs;
}
