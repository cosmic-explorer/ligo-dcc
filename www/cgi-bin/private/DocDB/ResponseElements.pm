...
require "DocumentSQL.pm";  #FIXME: Remove, move references to correct place
...
sub WarnPage () { # Non-fatal errors
  if (@WarnStack) {
    ...
    if ($#errors) {
      print "<dt class=\"Warning\">Warnings: </dt>\n";
    } else {
      print "<dt class=\"Warning\">Warning: </dt>\n";
    }
...
sub ErrorPage { # Fatal errors, continues page
  if (@ErrorStack) {
    ...
    if ($#errors) {
      print "<dt class=\"Error\">There were errors processing your
             request:</dt>\n";
    } else {
      print "<dt class=\"Error\">There was a error processing your
             request:</dt>\n";
    }
...
sub FullDocumentID ($;$) {
  ...
  if (defined $Version) {
    if ($Version == 0) {
        return "$ShortProject-$DocumentNumber-x$Version";
    }
    else {
        return "$ShortProject-$DocumentNumber-v$Version";
    }
  } else {
    return "$ShortProject-$DocumentNumber";
  }
}
...
sub DocumentURL {
  ...
  my $URL;
  if (defined $Version) {
    $URL = "$host_name/".FullDocumentID($DocumentID,$Version);
  } else {
    $URL = "$host_name/".FullDocumentID($DocumentID);
  }
  return $URL
}
