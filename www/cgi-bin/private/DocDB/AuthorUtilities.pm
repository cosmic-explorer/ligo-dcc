...
sub AuthorGroupRevIDsToAuthorGroupIDs {
  my ($ArgRef) = @_;
  ...
  foreach my $AuthorGroupRevID (@AuthorGroupRevIDs) {
    push @AuthorGroupIDs, $RevisionAuthorGroups{$AuthorGroupRevID}{AuthorGroupID};
  }
...
sub FirstAuthorGroupID ($) {
  ...
  unless (@AuthorGroupIDs) {return undef;}
  
  my $FirstID     = $AuthorGroupIDs[0];
