...
sub EventHeader ($) {
  ...
  my $SeparatorID = exists $ArgRef->{-separatorid} ? $ArgRef->{-separatorid} : 0;
  my $DisplayMode = exists $ArgRef->{-displaymode} ? $ArgRef->{-displaymode} : "";
  my $ListingType = exists $ArgRef->{-listingtype} ? $ArgRef->{-listingtype} : ""; ###
...
sub PrintMeetingEpilogue ($) {
  ...
  if ($Conferences{$ConferenceID}{Epilogue}) {
    ...
    print '<tr class="alt"><th>Event Wrapup:</th>';
    print "<td>\n";
...
sub EventsByGroup (%) {
  ...
  if ($Mode eq "display") {
     if ($SingleGroup) { # Format is a wee bit different for the single group form
        print "<div class=\"eventgroupid\">$Big<a href=\"$ListBy?eventgroupid=$EventGroupID\">$EventGroups{$EventGroupID}{ShortDescription}</a>$EBig</div>\n";
     } else {
        print "<div class=\"eventgroupid2\">$Big<a href=\"$ListBy?eventgroupid=$EventGroupID\">$EventGroups{$EventGroupID}{ShortDescription}</a>$EBig</div>\n";
     }
  } else {
    print "<div class=\"eventgroupid\">$Big$EventGroups{$EventGroupID}{ShortDescription}$EBig</div>\n";
  }
  
  print "<table class=\"$TableClass\">";
  if ($SingleGroup) { # Add end date and location for singe group display
     # Phil's table header!
     my $title = 'title="Click To Sort"';
     print "<thead><tr><th class=\"fancy\"><span $title>Event</span></th><th class=\"fancy\" sort=\"date\"><span $title>Start Date</th>";
     print "<th sort=\"date\" class=\"fancy\"><span $title>End Date</span></th><th class=\"fancy\"><span $title>Event Location</span></th>";
  } else {
     print "<thead><tr><th class=\"plain\">Event</th><th class=\"plain\">Start Date</th>";
  }
  print "</tr></thead>";
  my $EventCount = 0;
  my $Truncated = $FALSE;
  foreach my $EventID (@DisplayEventIDs) {
     ...
     if ($EventCount > $MaxEvents && $MaxEvents) { # Put ...show all... at bottom
        $Truncated = $TRUE;
        print '<td class="moreevents" colspan="2">';
        if ($Mode eq "display") {
           print "<a class=\"moreevents\" href=\"$ListAllMeetings?eventgroupid=$EventGroupID\">...more events and information...</a>\n";
        } else {
           print "<a class=\"moreevents\" href=\"$ListAllMeetings?eventgroupid=$EventGroupID&amp;mode=modify\">...more events and information...</a>\n";
        }
        print "</td>";
        last;
     } else { # Print normal entry
        print "<td class=\"meetinglink\">$MeetingLink</td>\n";
        print "<td class=\"startdate\">",EuroDate($Conferences{$EventID}{StartDate}),"</td>\n";

        if ($SingleGroup) { # Add end date and location for singe group display
           if ($Conferences{$EventID}{StartDate} ne $Conferences{$EventID}{EndDate}) {
              print "<td class=\"enddate\">".EuroDate($Conferences{$EventID}{EndDate})."</td>\n";
           } else {
              print "<td></td>\n";
           }
           print "<td class='eventlocation'>",$Conferences{$EventID}{Location},"</td>";
        }
     }
     print "</tr>\n";
  ...
  if (!$Truncated && !$SingleGroup) {
     print '<tr><td class="moreevents" colspan="2">';
     if ($Mode eq "display") {
        print "<a class=\"moreevents\" href=\"$ListAllMeetings?eventgroupid=$EventGroupID\">...more information...</a>\n";
     } else {
        print "<a class=\"moreevents\" href=\"$ListAllMeetings?eventgroupid=$EventGroupID&amp;mode=modify\">...more information...</a>\n";
     }
     print "</td></tr>";
  }
  print "</table>\n";
  if ($SingleGroup) { print "<p/>\n"; }
} # End of EventGroup
