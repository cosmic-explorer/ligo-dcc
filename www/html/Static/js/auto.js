...
(function($){
	...
	function setCharSize(data){
		for( var ch in data.chars ){
			if( ch == ' ' ) $(data.clone).html("<span id='test-width_"+data.id+"' style='display:inline-block'>&nbsp;</span>");
			else $(data.clone).html("<span id='test-width_"+data.id+"' style='display:inline-block'>"+ch+"</span>");
	...
	function createClone(id){
		...
		//i dont know why by chrome adds some pixels to the clientWidth...
...
//		if( text.charAt(text.length-1) == ' ' || text.charAt(text.length-1) == '\n' ) return "";
		...
		while( wordsFound < data.wordCount && pos >= 0 && text.charAt(pos) != '\n'){
			...
			pos--;
//			if( text.charAt(pos) == ' ' || pos < 0 ){   RDW Dec 12
			if( pos < 0 ){
				wordsFound++;
			}
		}
...
// fixed Aug 2013 RDW flip the quotes so that O'Reilly is allowed
...
//      console.log("--st" + seletedText);
//      if($seletedText == "... too many"){
//        return;
//      }
